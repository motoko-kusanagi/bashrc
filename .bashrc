...
echo -e "  "
echo -e "  \033[34mHDD /\e[0m : `df -h / | grep dev | awk {'print $3 " / " $2 " (" $5 ")"'}`\e[0m \t \033[34mMEM\e[0m : `free -m | grep Mem: | awk {'print $3'}`MB / `awk '/MemTotal/ {printf( "%.f\n", $2 / 1024 )}' /proc/meminfo`MB"
echo -e "  \033[34mIP\e[0m : `ifconfig $net1 | grep inet | head -1 | awk {' print $2'} | sed s/addr://g`\e[0m \t\t \033[34mSWAP\e[0m : `free -m | grep Swap: | awk {'print $3'}`MB / ` awk '/SwapTotal/ {printf( "%.f\n", $2 / 1024 )}' /proc/meminfo`MB"
